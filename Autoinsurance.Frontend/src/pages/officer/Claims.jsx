    ...
    <OfficerLayout>
      <div className="mx-auto px-4 py-4" style={{ maxWidth: 1000 }}>
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h1 className="font-serif text-dark m-0" style={{ fontSize: 26, fontWeight: 400 }}>Claims</h1>
        </div>
        ...
        {loading ? <LoadingSpinner /> : claims.length === 0 ? <EmptyState title="No claims found" description="There are no claims in the system." /> : (
            ...
            <table className="table table-hover mb-0" style={{ fontSize: 14 }}>
              <thead>
                <tr className="text-secondary text-uppercase" style={{ fontSize: 12, letterSpacing: '0.04em', background: '#F5EDE0' }}>
                  <th className="py-3 px-4 fw-semibold border-bottom-0">Claim No</th>
                  <th className="py-3 px-4 fw-semibold border-bottom-0">Policy No</th>
                  <th className="py-3 px-4 fw-semibold border-bottom-0">Amount</th>
              ...
              <tbody>
                {filteredClaims.map((c) => (
                  <tr key={c.claimId} className="align-middle border-top border-border">
                    <td className="py-3 px-4 text-dark fw-medium">{c.claimNumber}</td>
                    <td className="py-3 px-4 text-secondary">{c.policyNumber || `Policy #${c.policyId}`}</td>
                    <td className="py-3 px-4 text-secondary">₹{c.claimAmount}</td>
                    <td className="py-3 px-4"><StatusBadge status={c.claimStatus} /></td>
                    <td className="py-3 px-4">
                      <button onClick={() => navigate(`/officer/claims/${c.claimId}`)} className="btn btn-outline-primary btn-sm fw-semibold px-3">
                        Review
                      </button>
                    </td>
                  </tr>
